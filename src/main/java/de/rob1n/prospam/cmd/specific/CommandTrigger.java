...
public abstract class CommandTrigger extends Command
{
	...
	public void showGui(Player player, final String filter, final HashMap<Integer, List<String>> triggers)
    {
        ...
        for (int i = 0; i < 9; i = i + 2)
        {
            ...
            if(violationCount == 0)
            {
                items.add(new Item(i, new ItemStack(Material.LEGACY_SKULL_ITEM), "EVERY OTHER time", "Executed on all other occasions", Item.NO_CLICK_ACTION));
            }
            else
            {
                items.add(new Item(i, new ItemStack(Material.LEGACY_SKULL_ITEM), "At " + violationCount + ". time", "Executed on " + violationCount + ". violation", Item.NO_CLICK_ACTION));
            }

            items.add(new Item(i + 2 * 9, new ItemStack(Material.LEGACY_BOOK_AND_QUILL), "Edit commands", new Item.ClickAction()
            {
                @Override
        ...
        //back item
        items.add(new Item(27, new ItemStack(Material.LEGACY_ENDER_PORTAL), "Back to filter actions", new Item.ClickAction()
        {
            @Override
            public void onClick(Player player)
            {
	...
	private void showEditCommandsGui(Player player, final String filter, final HashMap<Integer, List<String>> triggers, final int violationKey)
    {
        ...
        if(triggers.get(violationKey) != null)
        {
            ...
            for (final String cmd : cmds)
            {
                ...
                items.add(new Item(iter++, new ItemStack(Material.LEGACY_RECORD_8), cmd, "Click to remove", new Item.ClickAction()
                {
                    @Override
                    public void onClick(Player player)
                    {
